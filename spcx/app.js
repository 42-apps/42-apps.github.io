...
function renderIntel(s) {
  ...
  if (company) {
    ...
    const src = [];
    if (c.sources && c.sources.massive) src.push('Massive');
    if (c.sources && c.sources.finnhub) src.push('Finnhub');
    $('companySrc').textContent = src.join(' + ');
    const errs = [];
    if (c.errors) {
      if (c.errors.massive) errs.push('Massive — ' + c.errors.massive);
      if (c.errors.finnhub) errs.push('Finnhub — ' + c.errors.finnhub);
    }
    let note = '';
...
function drawShareCard(s) {
  ...
  // company
  y = sect('Company & float', y);
  y = tiles([
    ['Shares out', co.sharesOutstanding != null ? fmtNum(co.sharesOutstanding) : '—'],
    ['Float', co.float != null ? fmtNum(co.float) : '—'],
    ['% float', co.floatPct != null ? co.floatPct + '%' : '—'],
    ['Mkt cap', co.marketCap != null ? '$' + fmtNum(co.marketCap) : '—'],
  ], y + 8, 4) + 14;
